...
//   useEffect(() => {
//     let cleanup = cb();
//     const unsub =  client.onInvalidate(hash => hashes.includes(hash) && (typeof cleanup === "function" && cleanup(), cb()));
//     return () => {
//       unsub();
...
//       if (stopped) return;
//       try {
//         await client.subscribe<SData, SVars>(subscription).forEach((res, cancel) => {
//           stopsub = cancel;
